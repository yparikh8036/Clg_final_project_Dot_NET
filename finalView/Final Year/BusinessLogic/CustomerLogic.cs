...
namespace Final_Year.BusinessLogic
{
    public class CustomerLogic
    {
        ...
        public static DataTable SelectByPKWithEmpName(int ID)
        {
            string query = "select c.*, e.Name from Customer c left join Employee e on c.EmployeeID= e.EmployeeID where c.CustomerID= @ID";
            List<SqlParameter> parameters = new List<SqlParameter>();
            parameters.Add(new SqlParameter("ID", ID));
            DataTable dt = DBHelper.SelectData(query, parameters);
            return dt;
        }
        public static DataTable SelectByManagerID(int ID)
        {
            string query = "select * from Customer where EmployeeID = @ID";
            List<SqlParameter> parameters = new List<SqlParameter>();
            parameters.Add(new SqlParameter("ID", ID));
            DataTable dt = DBHelper.SelectData(query, parameters);
            return dt;
